...
def test_krx_daily_index_quotation(
    ...
    resp = mock_openkrx_client.get_krx_daily_index(
        bas_dd="20240514",
    )
...
@responses.activate
def test_kospi_daily_index(
    mock_openkrx_client: OpenKrxClient, mock_openkrx_client_url: str
):
    expected = _generate_expected("KOSPI")
    ...
    resp = mock_openkrx_client.get_kospi_daily_index(
        bas_dd="20240514",
    )
...
@responses.activate
def test_kosdaq_daily_index(
    mock_openkrx_client: OpenKrxClient, mock_openkrx_client_url: str
):
    expected = _generate_expected("KOSDAQ")
    ...
    resp = mock_openkrx_client.get_kosdaq_daily_index(
        bas_dd="20240514",
    )
...
def test_bond_daily_index(
    ...
    responses.add(
        responses.GET,
        f"{mock_openkrx_client_url}/svc/apis/idx/bon_dd_trd",
        json=expected,
        status=200,
    )

    resp = mock_openkrx_client.get_bond_daily_index(
        bas_dd="20240514",
    )
...
def test_derivatives_daily_index(
    ...
    responses.add(
        responses.GET,
        f"{mock_openkrx_client_url}/svc/apis/idx/drvprod_dd_trd",
        json=expected,
        status=200,
    )

    resp = mock_openkrx_client.get_derivatives_daily_index(
        bas_dd="20240514",
    )
