...
def test_kospi_base_info(integration_openkrx_client: OpenKrxClient):
    resp = integration_openkrx_client.get_kospi_base_info(bas_dd="20240514")

    assert resp is not None
    assert len(resp["OutBlock_1"]) == 953


def test_kosdaq_base_info(integration_openkrx_client: OpenKrxClient):
    resp = integration_openkrx_client.get_kosdaq_base_info(bas_dd="20240514")

    assert resp is not None
    assert len(resp["OutBlock_1"]) == 1728


def test_konex_base_info(integration_openkrx_client: OpenKrxClient):
    resp = integration_openkrx_client.get_konex_base_info(bas_dd="20240514")
